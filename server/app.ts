...
import { processOMRSheet } from './omr_processor';
import { generateExamExcel } from './excel_exporter';
import { evaluateHandwritingWithGemini, SYSTEM_PROMPT_HANDWRITING_OCR } from './gemini';
...
// Initialize Neon PostgreSQL Database (Lazy/Background)
...
// Direct alias for /api/db-status
app.get('/api/db-status', async (req, res) => {
  const status = await testConnection();
  res.json(status);
});
